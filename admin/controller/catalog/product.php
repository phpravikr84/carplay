...
class ControllerCatalogProduct extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			$data['products'][] = array(
				...
				'category_name' => implode(", ",$this->model_catalog_product->getProductCategoriesName($result['product_id'])),
				'image'      => $image,
				'name'       => $result['name'],
		...
		$data['sort_minute'] = $this->url->link('catalog/product', 'token=' . $this->session->data['token'] . '&sort=p.minute' . $url, true);
	...
	protected function getForm() {
		...
		if (isset($this->request->post['image']) && is_file(DIR_IMAGE . $this->request->post['image'])) {
			...
			$splitproduct = explode('_',$product_info['image']);
			$funcproduct = $splitproduct[0];
			if(count($splitproduct) > 1)
  			$data['thumb'] = "https://s3.amazonaws.com/riwigo-storage/images/img-thumb/".$splitproduct[1];
			else
  			$data['thumb'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		...
		foreach ($product_images as $product_image) {
			...
			if (is_file($product_image['image'])) {
				...
				$splitthumb = explode('_',$product_image['image']);
				$functhumb = $splitthumb[0];
				if(count($splitthumb) > 1)
  				$thumb = "https://s3.amazonaws.com/riwigo-storage/images/img-thumb/".$splitthumb[1];
				else
 	 			$thumb = 'no_image.png';
		...
		foreach ($product_discounts as $product_discounts) {
			...
			$data['product_discounts'][] = array(
				'product_id'      	=> $product_discounts['product_id'],
