...
class ModelCatalogMerchant extends Model {
	public function addMerchant($data) {
       ...
       $sql = "INSERT INTO " . DB_PREFIX . "merchant SET "
			...
			. "avg_discount = '" . (int)$data['avg_discount'] . "', " 
			. "avg_seats = '" . (int)$data['avg_seats'] . "', " 
			...
			."merchant_type = '" . (int)$data['merchant_type'] . "', " 
			."margin_value = '" . (int)$data['margin_value'] . "', " 
			."margine_type = '" . $this->db->escape($data['margine_type']) . "', " 
		...
		//Merchant Discount
		if (isset( $data['booking_start_date'] ) && isset( $data['booking_end_date'])){ 
		
				$begin = new DateTime( $data['booking_start_date'] );
				$end = new DateTime( $data['booking_end_date'] );
				...
				foreach ( $period as $dt ){
					...
					foreach($range as $time){
						...
						$start = $dt->format( "Y-m-d" ).' '.date("H:i",$time);
						$end = $dt->format( "Y-m-d" ).' '.date("H:i",strtotime('+30 minutes',$time));
						 
						$sql = "INSERT INTO " . DB_PREFIX . "merchant_disc SET "
						. "title = 'Discount', " 
						...
						. "allDay = 'No', "  
						. "url = '?page=', " 
						. "discount = '".(int)$data['avg_discount']."', " 
						. "merchant_id = '" . $merchant_id . "', " 
						. "seats = '".(int)$data['avg_seats']."'";
		...
		//Merchant Spoken Languages 
		if (isset($data['merchant_mcategory'])) {
			foreach ($data['merchant_mcategory'] as $mcategory_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "merchant_to_mcategory SET merchant_id = '" . (int)$merchant_id . "',   mcategory_id = '" . (int)$mcategory_id . "'");
			}
		} 
		//Merchant Image
	...
	public function editMerchant($merchant_id, $data) {
		...
		$sql = "SELECT COUNT(DISTINCT id) AS total FROM " . DB_PREFIX . "merchant_disc WHERE merchant_id = '" . (int)$merchant_id. "'";
		...
		$query= $this->db->query($sql);
		 ...
		 if ($query->row['total']==0){  
			...
			$sql = "SELECT MAX(start) as start FROM " . DB_PREFIX . "merchant_disc WHERE merchant_id = '" . (int) $merchant_id. "'";
			
			$query= $this->db->query($sql);

		 	//print date('Y-m-d', strtotime($query->row['start']  . ' +1 day' ));exit;
			
			$data['booking_start_date'] = date('Y-m-d', strtotime($query->row['start']  . ' +1 day' ));
			
			if($data['booking_end_date'] > $data['booking_start_date'])  {
        ...
        //Merchant Services
		//print '<pre>'; print_r($data['merchant_service']); exit;
				...
				if( $merchant_discount == 0){ 
					...
					foreach ( $period as $dt ){
						...
						foreach($range as $time){
							...
							$sql = "INSERT INTO " . DB_PREFIX . "merchant_disc_product_wise SET "
							. "title = 'Discount', " 
							. "description = 'Discount', " 
							. "start = '" . $this->db->escape($start) . "', " 
							. "product_id = '" . (int)($merchant_service['service_id']) . "',"
		...
		if (isset($data['merchant_mcategory'])) {
			$this->db->query("DELETE FROM " . DB_PREFIX . "merchant_to_mcategory WHERE merchant_id = '" . (int)$merchant_id . "'");
			foreach ($data['merchant_mcategory'] as $mcategory_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "merchant_to_mcategory SET merchant_id = '" . (int)$merchant_id . "',   mcategory_id = '" . (int)$mcategory_id . "'");
			}
		} 
	...
	public function deleteMerchant($merchant_id) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "merchant_disc WHERE merchant_id = '" . (int)$merchant_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "url_alias WHERE query = 'merchant_id=" . (int)$merchant_id . "'"); 
		 
		$this->cache->delete('merchant');
	}
	
	public function removeCalender() {
		
		 $this->db->query("DELETE FROM  calendar");
	}
	
	public function updateCalender($merchant_id) {
		
		 $this->db->query("DELETE FROM  calendar");
		 
		 $this->db->query("INSERT INTO `calendar` SELECT * FROM " . DB_PREFIX . "merchant_disc WHERE merchant_id='" . (int)$merchant_id . "'");
	...
	public function getMerchants($data = array()) {
		...
		if (isset($data['filter_countryname']) && !is_null($data['filter_countryname'])) {
			$sql .= " AND pd.country_id = '" . (int)$data['filter_countryname'] . "'";
		}

		if (isset($data['filter_cityname']) && !is_null($data['filter_cityname'])) {
			$sql .= " AND pd.city_id = '" . (int)$data['filter_cityname'] . "'";
		}
	...
	public function getMcategories($data = array()) {
		...
		if (!empty($data['filter_mcategory_id'])) {
			$sql .= " AND mcategory_id = '" . $this->db->escape($data['filter_mcategory_id']) . "'";
		} 
		
		 
		 
		 
		$sql .= " GROUP BY  mcategory_id";
	...
	public function getgetMcategories($data = array()) {
		$sql = "SELECT * FROM " . DB_PREFIX . "mcategory WHERE language_id = '" . (int)$this->config->get('config_language_id') . "'";
		...
		if (!empty($data['filter_spokenlanguage_id'])) {
			$sql .= " AND  mcategory_id  = '" .  (int) $data['filter_mcategory_id'] . "'";
		} 
	...
	public function getMerchantAll() {
            $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "merchant p LEFT JOIN " . DB_PREFIX . "merchant_description pd ON (p.merchant_id = pd.merchant_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY pd.name ASC");
            return $query->rows;
	}
    ...
    public function getTotalMerchantServiceWiseDiscount($merchant_id, $product_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "merchant_disc_product_wise WHERE merchant_id = '" . (int)$merchant_id . "' and product_id = '".$product_id."'");
	...
	public function getMcategoriesByMerchantId($merchant_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "merchant_to_mcategory WHERE merchant_id = '" . (int)$merchant_id . "'");

		return $query->rows;
	}
	
	public function autocompleteRecomondedProducts($data = array()) {
		
		
		$sql = "SELECT pd.name, pd.product_id, mp.duration, mp.price FROM " . DB_PREFIX . "merchant m";
		$sql .= " LEFT JOIN " . DB_PREFIX . "merchant_products mp ON (m.merchant_id = mp.merchant_id) ";
		$sql .= " LEFT JOIN " . DB_PREFIX . "product_description pd ON (mp.product_id = pd.product_id) ";
		$sql .= " WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND m.merchant_id='".$data['filter_merchant_id']."'";
		
		if (!empty($data['filter_name'])) {
			$sql .= " AND pd.name LIKE %'".$data['filter_category_id']."'%";
		}
