...
class ModelExtensionModule extends Model {
	...
	public function editModule($module_id, $data) { //print '<pre>'; print_r($data);exit;
		$sql ="UPDATE `" . DB_PREFIX . "module` SET `name` = '" . $this->db->escape($data['name']) . "', `setting` = '" . $this->db->escape(json_encode($data)) . "' WHERE `module_id` = '" . (int)$module_id . "'";
		//print $sql;exit;
		$this->db->query($sql);
	}
