...
class ModelReportSale extends Model {
	public function getTotalSales($data = array()) {  
		...
		if(isset($data['filter_merchant_id'] ) && $data['filter_merchant_id'] !='0'){
			$sql .= "AND merchant_id  = '".$data['filter_merchant_id']."' ";
		}
	...
	public function getTotalSaless($data = array()) {  
		...
		if(isset($data['filter_merchant_id'] ) && $data['filter_merchant_id'] !='0' && $data['filter_merchant_id'] !=''){
			$sql .= "AND merchant_id  = '".$data['filter_merchant_id']."' ";
		}
	...
	public function getTotalOrdersByCountry() {
		...
		$sql = "SELECT COUNT(*) AS total, SUM(o.total) AS amount, c.iso_code_2 FROM `" . DB_PREFIX . "order` o LEFT JOIN `" . DB_PREFIX . "country` c ON (o.payment_country_id = c.country_id) ";
		$sql .= " WHERE o.order_status_id > '0' ";
		if($filter_merchant_id !=''){
			$sql .= " AND o.merchant_id = '".$filter_merchant_id."' ";
		}
	...
	public function getTotalOrdersByDay() {
		...
		$sql = "SELECT COUNT(*) AS total, HOUR(date_added) AS hour FROM `" . DB_PREFIX . "order` WHERE ";
		$sql .= "order_status_id IN(" . implode(",", $implode) . ") AND DATE(date_added) = DATE(NOW()) ";
		if(isset($filter_merchant_id) && $filter_merchant_id !=0){
			$sql .= "AND merchant_id  = '".$filter_merchant_id."' ";
		}
	...
	public function getTotalOrdersByMonth() {
		...
		$sql ="SELECT COUNT(*) AS total, date_added FROM `" . DB_PREFIX . "order` ";
		$sql .=" WHERE order_status_id IN(" . implode(",", $implode) . ") ";
		
		if(isset($filter_merchant_id ) && $filter_merchant_id !=''){
			$sql .= "AND merchant_id  = '".$filter_merchant_id."' ";
		}
		
		$sql .=" AND DATE(date_added) >= '" . $this->db->escape(date('Y') . '-' . date('m') . '-1') . "' GROUP BY DATE(date_added)";	
	...
	public function getOrders($data = array()) {
		$sql = "SELECT MIN(o.date_added) AS date_start, MAX(o.date_added) AS date_end, COUNT(*) AS `orders`, SUM((SELECT SUM(ot.value) FROM `" . DB_PREFIX . "order_total` ot WHERE ot.order_id = o.order_id AND ot.code = 'tax' GROUP BY ot.order_id)) AS tax, SUM(o.total) AS `total` FROM `" . DB_PREFIX . "order` o";
