...
<table id="discount" class="table table-striped table-bordered table-hover">
  <thead>
    <tr>
      ...
      <?php
                     ...
                     // End date
                     $end_date =  date('Y-m-d', strtotime($booking_start_date. ' + 6 days'));
                     ?>
  ...
  <tbody >
    <?php $discount_row = 0; ?>
    <?php foreach ($merchant_discounts as $merchant_discount) {  $i=0; $hideTd=1;?>
    <tr id="discount-row<?php echo $discount_row; ?>">
      ...
      <td class="text-right"><input type="text" name="merchant_discount[<?php echo $discount_row; ?>][disc_data][<?php echo $i;?>][seats]" value="<?php echo $merchant_disc['seats']; ?>" placeholder="<?php echo $entry_quantity; ?>" class="form-control"  onchange="calcDiscount(<?php echo $discount_row; ?>)"/>
        ...
        <input type="text" name="merchant_discount[<?php echo $discount_row; ?>][disc_data][<?php echo $i;?>][merchant_discount_id]" value="<?php echo  $merchant_disc['merchant_discount_id']; ?>" placeholder="" class="form-control" /></td>
      <?php $i++;  $hideTd++; } ?>
