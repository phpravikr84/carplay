<div class="modal-dialog modal-lg">
 ...
 <?php 
			error_reporting(0);
           	...
           	$imgArray = array();
           	$loop=1;
           	foreach (array_chunk($simg, 4) as $image){
...
<div class="row">
			...
			<div class="col-sm-3 text-center">
          	<a href="<?php echo $image['name']; ?>" class="thumbnail">
          		
          	<img src="https://s3.amazonaws.com/riwigo-storage/images/img-thumb/<?php //echo $s3imgname[1]; 
          	$s3imgname = explode('_', $image['name']);
          	echo $s3imgname[1];  
          	?>" alt="<?php echo $image['name']; ?>" title="<?php echo $image['name']; ?>" /></a>
          	 
          	<label>
            <input type="checkbox" name="path[]" value="https://s3.amazonaws.com/riwigo-storage/<?php echo $image['name']; ?>" />
            <?php $s3imgname_thumb = explode('_', $image['name']);
          	echo $s3imgname_thumb[1];  ?></label>
          </div>
...
$('a.thumbnail').on('click', function(e) {
	...
	var imgPath = "https://s3.amazonaws.com/riwigo-storage/"+$(this).find('img').attr('alt');
	//alert(imgPath);
	...
	$('#<?php echo $thumb; ?>').find('img').attr('alt', $(this).find('img').attr('alt'));
	<?php } ?>
	//$(this).parent().find('input').attr('value')=$(this).find('img').attr('alt');
	$('#<?php echo $target; ?>').attr('value', $(this).parent().find('input').attr('value'));
	$('#input-image').attr('value', imgPath);
...
$('#button-upload').on('click', function() {
	...
	$('body').prepend('<form enctype="multipart/form-data" name="image" id="form-upload" style="display: none;"><input type="file" name="file" value="" /></form>');
