...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
		...
		margin_type = '" . $this->db->escape($data['margin_type']) . "',
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
				...
				price = '" .  ($product['bookingActualPrice'] ). "', 
				total = '" .  ($product['bookingActualPrice']) * $product['persons'] . "', 
				tax = '" .  ($product['tax']) . "', 
				...
				$totalAmount = ($product['bookingActualPrice']) * $product['persons'];
				$creditAmount = $totalAmount - $product['disc_amount'];
				$this->db->query("INSERT INTO " . DB_PREFIX . "transacation SET merchant_id = '" . (int)$product['merchant_id'] . "', order_id = '" . (int)$order_id . "', 
				dr_amount = '',
				cr_amount = '" . (int)$creditAmount . "', remarks ='OrderId-" . (int)$order_id . "', date_added = NOW(), status=1");

				
			}
		}
		
		$this->db->query("UPDATE " . DB_PREFIX . "merchant_disc SET used_seats = used_seats + 1  WHERE id = '" . (int)$data['bookingDisocuntId'] . "'");
		...
		if (isset($data['totals'])) {
			foreach ($data['totals'] as $total) {
				...
				title = '" . $this->db->escape($total['title']) . "', `value` = '" . round($total['value']) . "', 
	...
	public function deleteOrder($order_id) {
		...
		$this->db->query("DELETE FROM `" . DB_PREFIX . "transacation` WHERE order_id = '" . (int)$order_id . "'");
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) { 
		...
		if ($order_info) {
			...
			// Update Count Reservation in Merchant Table
			$this->db->query("UPDATE `" . DB_PREFIX . "merchant` SET reserved = reserved + 1  WHERE merchant_id = '" . (int)$order_info['merchant_id'] . "'");
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				foreach ($order_product_query->rows as $product) {
					...
					$bkDate = $this->db->query("SELECT * FROM " . DB_PREFIX . "order WHERE order_id = '" . (int)$product['order_id'] . "' GROUP BY order_id");
					...
					foreach($bkDate->rows as $bookingDate){
						$booking=$bookingDate['booking_date'];
						$purchaseDate=date("d-m-Y", strtotime($booking));
						if($bookingDate['payment_code']=='cod'){
