<?php
class ModelWebserviceCheckoutOrder extends Model {
	public function addOrder($data) {
		...
		store_url = '" . $this->db->escape($data['store_url']) . "', merchant_id = '" . (int)$data['merchant_id'] . "', 
		...
		telephone = '" . $this->db->escape($data['telephone']) . "', mobile = '" . $this->db->escape($data['mobile']) . "', 
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
				...
				persons = '" . $this->db->escape($product['persons']) . "',
				bookingDiscountTime = '" . $this->db->escape($product['bookingDiscountTime']) . "',
				disc_amount = '" . (int)$product['disc_amount'] . "',
				sub_total = '" . (int)$product['sub_total'] . "',
		...
		if (isset($data['totals'])) {
			foreach ($data['totals'] as $total) {
				...
				order_id = '" . (int)$order_id . "', code = '" . $this->db->escape($total['code']) . "', 
				title = '" . $this->db->escape($total['title']) . "', `value` = '" . (float)$total['value'] . "', 
	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			return array(
				...
				'merchant_id'                => $order_query->row['merchant_id'],
	...
	public function getOrderApi($order_id) {
		...
		if ($order_query->num_rows) {
			...
			return array(
				...
				'currency_code'           => $order_query->row['currency_code'],
				'currency_value'          => $order_query->row['currency_value'],
				'booking_date'              => $order_query->row['booking_date'],
				'date_added'              => $order_query->row['date_added'],
				'date_modified'           => $order_query->row['date_modified']
			);
		} else {
	...
	public function getOrderApiNew($order_id) {
		$order_query = $this->db->query("SELECT *, (SELECT os.name FROM `" . DB_PREFIX . "order_status` os WHERE os.order_status_id = o.order_status_id AND os.language_id = o.language_id) AS order_status FROM `" . DB_PREFIX . "order` o WHERE o.order_id = '" . (int)$order_id . "'");
		...
		if ($order_query->num_rows) {
			...
			$merchant_query= $this->db->query("SELECT * FROM " . DB_PREFIX . "merchant m LEFT JOIN " . DB_PREFIX . "merchant_description md ON m.merchant_id=md.merchant_id WHERE m.merchant_id= '" . (int)$order_query->row['merchant_id'] . "'GROUP BY m.merchant_id");

			//print_r($merchant_query->rows);
			//exit;

			if ($merchant_query->num_rows) {
				$merchant_name = $merchant_query->row['name'];
				$merchant_address =  $merchant_query->row['address'];
				$merchant_thumb = $merchant_query->row['image'];
				$merchant_latitude =  $merchant_query->row['latitude'];
				$merchant_longitude =  $merchant_query->row['longitude'];
			} else {
				$merchant_name = '';
				$merchant_address =  '';
				$merchant_thumb = '';
			...
			$currency_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "currency WHERE currency_id='".(int)$order_query->row['currency_id']."'");

			if($currency_query->num_rows){
				$currency_symbol = $currency_query->row['symbol_left'];
			} else{
				$currency_symbol = '';
			}
	...
	public function getReservationApi($order_id) {
		$order_query = $this->db->query("SELECT *, (SELECT os.name FROM `" . DB_PREFIX . "order_status` os WHERE os.order_status_id = o.order_status_id AND os.language_id = o.language_id) AS order_status FROM `" . DB_PREFIX . "order` o WHERE o.order_id = '" . (int)$order_id . "'");
		...
		if ($order_info) {
			...
			if (!in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
				...
				/*foreach ($order_product_query->rows as $order_product) {
					$this->db->query("UPDATE " . DB_PREFIX . "product SET quantity = (quantity - " . (int)$order_product['persons'] . ") WHERE product_id = '" . (int)$order_product['product_id'] . "' AND subtract = '1'");
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				// $data['text_terms'] = $language->get('text_terms');
				// $data['text_policies'] = $language->get('text_policies');
				...
				$data['cancellink'] = $order_info['store_url'] . 'index.php?route=account/order/cancel&order_id=' . $order_id;
				$data['termsandcondition'] = $order_info['store_url'] . 'index.php?route=information/information&information_id=5';
				$data['policies'] = $order_info['store_url'] . 'index.php?route=information/information&information_id=3';
				...
				$data['telephone'] = $order_info['mobile'];
				...
				$data['merchant_email'] = $merchant_info['email'];
				...
				foreach ($order_product_query->rows as $product) {
					$text .= $product['persons'] . 'x ' . $product['name'] . '   ' . html_entity_decode($this->currency->format($product['total'] + ($this->config->get('config_tax') ? ($product['tax'] * $product['persons']) : 0), $order_info['currency_code'], $order_info['currency_value']), ENT_NOQUOTES, 'UTF-8') . "\n";
