...
class ModelPaymentpaypalexpress extends Model {
	...
	public function SetExpressCheckout($products, $charges){
			...
			*/
			...
			$padata .=	'&PAYMENTREQUEST_0_CURRENCYCODE='.urlencode($item['currencyname']);
			
			//paypal custom template $item['currencyname']
			...
			$this->request->session['ppl_products'] =  $products;
			$this->request->session['ppl_charges'] 	=  $charges;
			...
			if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])){
				...
				/*https://www.sandbox.paypal.com/cgi-bin/webscr?cmd=_express-checkout&token=EC%2d8PB059446M0588050#/checkout/login*/
				//echo $paypalurl;
				//exit;
				//redirect($paypalurl);
				...
				// echo '</pre>';
				$output = '<div style="color:red"><b>Error : </b>'.urldecode($httpParsedResponseAr["L_LONGMESSAGE0"]).'</div>';
				$output .= '<pre>';
				$output .= $httpParsedResponseAr;
				return $output;
			}	
		}		
	...
	public function DoExpressCheckoutPayment(){
			...
			if(!empty($this->_SESSION('ppl_products'))&&!empty($this->_SESSION('ppl_charges'))){
				
				$products=$this->_SESSION('ppl_products');
				
				$charges=$this->_SESSION('ppl_charges');
				
				$padata  = 	'&TOKEN='.urlencode($this->_GET('token'));
				$padata .= 	'&PAYERID='.urlencode($this->_GET('PayerID'));
				$padata .= 	'&PAYMENTREQUEST_0_PAYMENTACTION='.urlencode("SALE");
	...
	public function GetTransactionDetails(){
			...
			if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])){
				...
				*/
				...
				$output = '<div style="color:red"><b>GetTransactionDetails failed:</b>'.urldecode($httpParsedResponseAr["L_LONGMESSAGE0"]).'</div>';
				$output .= '<pre>'.$httpParsedResponseAr.'</pre>';
