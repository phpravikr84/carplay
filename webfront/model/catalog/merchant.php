...
class ModelCatalogMerchant extends Model {
	...
	public function getMerchantMargin($merchant_id){

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "merchant WHERE merchant_id = '" . (int)$merchant_id . "' ORDER BY merchant_id");
	...
	public function getMerchantsByCategoryId($data = array()) { //print_r($data);
		...
		// } 	
		 ...
		 if (isset($data['filter_rating']) && !empty($data['filter_rating']) && $data['filter_rating']!=0) {
			
			if ($data['rating'] == 'minpricelevel') {
				$sql .= " ORDER BY  m.rating ASC";
			}
			if ($data['rating'] == 'maxpricelevel') {
		...
		if (isset($data['sort'])) {
			if ($data['sort'] == 'minprice_level') {
				$sql .= " ORDER BY  m.price_level ASC";
			}
			if ($data['sort'] == 'maxprice_level') {
				$sql .= " ORDER BY  m.price_level DESC";
			}
		} else {
			$sql .= " ORDER BY m.sort_order";
		} 	
