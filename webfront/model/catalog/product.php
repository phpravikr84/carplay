...
class ModelCatalogProduct extends Model {
	...
	public function getProfiles($product_id) {
        ...
        public function getMerchantsByProductsId($product_id,$city_id=10) {
			...
			//print "SELECT count(mp.merchant_id) as total FROM `" . DB_PREFIX . "merchant_products` mp LEFT JOIN `" . DB_PREFIX . "merchant_description` md on (mp.merchant_id = md.merchant_id) WHERE mp.product_id = " . (int)$product_id . " AND md.city_id = '".(int)$city_id."' "
			
		return $this->db->query("SELECT count(mp.merchant_id) as total FROM `" . DB_PREFIX . "merchant_products` mp LEFT JOIN `" . DB_PREFIX . "merchant_description` md on (mp.merchant_id = md.merchant_id) WHERE mp.product_id = " . (int)$product_id . " AND md.city_id = '".(int)$city_id."' AND md.language_id = '" . (int)$this->config->get('config_language_id')."'")->row['total'];
	}
