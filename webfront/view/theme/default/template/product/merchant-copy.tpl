<?php echo $header;  //print_r($allService);exit; ?>
              ...
              <table class="table">
        		...
        		<?php foreach ($merchant['recomonded'] as $recomonded) { ?>
                  <tr>
                    ...
                    <td><a href="<?=$merchant['href'];?>&product_id=<?php echo $recomonded['product_id']?>"><button type="button" class="btn btn-default">Select</button></a></td>
                  </tr>
                  <?php }?>
                  <?php }?> 
                </tbody>
               ...
               <a href="javascript:void(0)" data-toggle="modal" data-target="#myspaModal" title="<?php echo $merchant['merchant_id']; ?>" class="spamenu"><strong>See all services</strong><i class="fa" aria-hidden="true">
...
$('#drowdown-group-resto').on('change', function() {
	...
	var filter_product = $('select[name=\'drowdown-group-resto\']').val();

	if (filter_product != '*') {
		url += '&product_id=' + encodeURIComponent(filter_product);
	}
