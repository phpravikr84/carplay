...
<?php } ?>
<div id="registrationfront" class="register-container registerfront col-sm-12" style="padding-right:0px">
  <form action="" method="post" enctype="multipart/form-data" class="form-horizontal">
    ...
    <input type="text" name="countrycode" id="isdcodes" style="width:100px; height:46px;" class="form-control" tabindex="5"><input type="text" name="telephone" value="<?php echo $telephone; ?>" placeholder="<?php echo $telephone_lable;?>" id="telephone" class="form-control" style="width: 58%;"/>
    </div>
    <?php } ?>
    <?php if($fax_status==1){ ?>
    <div class="col-sm-6 form-group">
    ...
    <input type="hidden" name="redirect" value="<?php echo $redirect; ?>" placeholder="<?php echo $redirect;?>" id="redirect" class="form-control" />
      <input class="btn btn-primary form-control" type="button" value="<?php echo $button_submitbutton; ?>" id="signuppage"/>
    </div>
</div>
...
<div class="rightside">
    ...
    <div class="col-sm-12 social">
      ...
      <p><?php echo $text_account_alrdy; ?> <a class="regclickfront" rel="loginfront" rel1="registrationfront" rel2="forgotten"><?php echo $text_loginpage; ?></a></p>
      <?php } ?>
    </div>
</div>
...
<div id="loginfront" style="display:none"  class="login-container loginpanel col-sm-12">
  <form  action="" method="post" enctype="multipart/form-data" class="form-horizontal">
    ...
    <div class="form-group required">
      <div class="col-sm-12">
        ...
        <a class="regclickfront" rel="forgottenfront" rel1="registrationfront" rel2="loginfront"><?php echo $text_forgotten; ?></a>
        <?php } ?>
      </div>
    </div>
    <div class="form-group">
      <div class="col-sm-12">
       <input type="hidden" name="redirect" value="<?php echo $redirect; ?>" placeholder="<?php echo $redirect;?>" id="redirect" class="form-control" />
        <input class="btn btn-primary form-control" type="button" value="<?php echo $button_login; ?>" id="loginpagefirst"/>
      </div>
    </div>
  </div><!--end leftside-->
    ...
    <div class="col-sm-12 social">
      ...
      <p> <a class="regclickfront" rel="registrationfront"  rel1="loginfront" rel2="forgottenfront"><?php echo $text_regpage; ?></a></p>
...
<!-- Forget Code -->
<div id="forgottenfront" style="display:none"  class="forgotten-container forgotten col-sm-12">
  ...
  <div class="form-group required">
    <div class="col-sm-12">
      <input type="text" name="email" value="<?php echo $email; ?>" id="forgetemailfront" class="form-control" placeholder="Email"/>
      <span class="name-icon fui-mail gray"></span> </div>
  </div>
  ...
  <div class="form-group forgt">
    ...
    <!--   <div class="pull-left"> <a style="padding:10px;" class="click btn btn-default" rel="login" rel1="registration" rel2="forgotten"><?php echo $button_back; ?></a> </div> -->
      <div class="pull-right">
       <input type="hidden" name="redirect" value="<?php echo $redirect; ?>" placeholder="<?php echo $redirect;?>" id="redirect" class="form-control" />
        <!-- <input type="button" value="<?php echo $button_continue; ?>" class="btn btn-primary" id="forgettfront" style="padding:10px;"/> -->
          <input type="button" value="Submit" class="btn btn-primary" id="forgettfront" style="padding:10px;"/>
      </div>
    </div>
  </div>  
    </div>
...
<script>
 ...
 $(".regclickfront").click(function(){    
   ...
   $("#"+id2).hide();
    ...
    $('#signuppage').bind('click', function() {
  $.ajax({
  url: 'index.php?route=common/login/addregister',
  type: 'post',
  data: $('.registerfront input[type=\'text\'],.registerfront input[type=\'password\'], .registerfront input[type=\'hidden\'], .registerfront input[type=\'radio\']:checked, .registerfront input[type=\'checkbox\']:checked, .registerfront select, .registerfront textarea'),
  dataType: 'json',
  beforeSend: function() {
      $('#signuppage').button('loading');
    },
   complete: function() {
            $('#signuppage').button('reset');
        },  
  ...
  $('.alert, .text-danger').remove();
   ...
   $('#loginpagefirst').bind('click', function() {
  $.ajax({
  url: 'index.php?route=common/login/addlogin',
  type: 'post',
  data: $('.loginpanel input[type=\'text\'],.loginpanel input[type=\'password\'], .loginpanel input[type=\'hidden\'], .loginpanel input[type=\'radio\']:checked, .loginpanel input[type=\'checkbox\']:checked, .loginpanel select, .loginpanel textarea'),
  dataType: 'json',
  beforeSend: function() {
      $('#loginpagefirst').button('loading');
    },
   complete: function() {
            $('#loginpagefirst').button('reset');
        },  
  success: function(json) {
  $('.alert, .text-danger').remove();
  ...
  $('#forgettfront').bind('click', function() {
  $.ajax({
  url: 'index.php?route=common/login/forgetpwd',
  type: 'post',
  data: $('#forgottenfront input[type=\'text\']'),
  dataType: 'json',
  beforeSend: function() {
      $('#forgettfront').button('loading');
    },
   complete: function() {
            $('#forgettfront').button('reset');
        },  
  success: function(json) {
  $('.alert, .text-danger').remove();
    ...
    if (json['error']) {
      ...
      if(typeof (json['error']['forgetemailfront'])!='undefined'){
      $('#forgetemailfront').parent('div').before('<div class="alert alert-danger"> <i class="fa fa-exclamation-circle"></i> ' + json['error']['forgetemailfront'] + '<button type="button" class="close" style="background:none;color:#000;padding:0px;" data-dismiss="alert">&times;</button></div>');
      }
      $('.warning').fadeIn('slow');
    }
...
#quickSignupModal .modal-content{background-color:<?php echo $contanercolor?>}
#quickSignupModal h2{color:<?php echo $headingcolor?>}
#quickSignupModal select.form-control, #quickloginModal  input.form-control[type="text"],#quickSignupModal input.form-control[type="password"]{border-bottom:solid 1px <?php echo $inputbordercolor?>; color:<?php echo $textcolor?>;background-color:<?php echo $contanercolor?>;margin-bottom:10px;}
#quickSignupModal .btn{background:<?php echo $btnbgcolor?>!important;color:<?php echo $btncolor?>!important}
#quickSignupModal .socialtitle{color:<?php echo $scfontcolor?>!important}
...
.intl-tel-input.allow-dropdown input#isdcodes
{
