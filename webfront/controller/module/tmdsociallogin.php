...
class ControllerModuletmdSociallogin extends Controller {
	public function index($setting) { //print 'satnanu';exit;
		...
		require_once DIR_SYSTEM.'library/tmdsocial/src/Google_Client.php';
		require_once DIR_SYSTEM.'library/tmdsocial/src/contrib/Google_Oauth2Service.php';
		require_once DIR_SYSTEM.'library/tmdsocial/src/contrib/Google_PlusService.php';

		
		
		$appName='Riwigo';
$googleclientid ="162217844639-alq2nankhnh1s4piem71jgi4vddlg14b.apps.googleusercontent.com";
		$googleclientsecret ="rTejcByFeiEn6MPUCdjdg_p5";
		...
		$gClient->setClientSecret($googleclientsecret);
		 $gClient->setAccessType('online'); // Access method
	 ...
	 $gClient->setScopes(array('https://www.googleapis.com/auth/plus.login', 'https://www.googleapis.com/auth/plus.me', 'https://www.googleapis.com/auth/userinfo.email', 'https://www.googleapis.com/auth/userinfo.profile'));
		// $gClient->setRedirectUri($this->url->link('module/tmdsociallogin/googleredirect', '', 'SSL'));

	 $gClient->setRedirectUri("http://dev.riwigo.com/index.php?route=module/tmdsociallogin/googleredirect");
	 ...
	 $gClient->setScopes(array('https://www.googleapis.com/auth/plus.login', 'https://www.googleapis.com/auth/plus.me', 'https://www.googleapis.com/auth/userinfo.email', 'https://www.googleapis.com/auth/userinfo.profile'));
	  ...
	  $gClient->setRedirectUri("http://dev.riwigo.com/index.php?route=module/tmdsociallogin/googleredirect");
		...
		/* Google Login link code */
		 ...
		 $google_service = new Google_PlusService($gClient);

// $gClient->authenticate();
//  $_SESSION['<span id="IL_AD7" class="IL_AD">token</span>'] = $gClient->getAccessToken();
// if (isset($this->session->data['token'])) {
//  $set_asess_token = $gClient->setAccessToken($this->session->data['token']);
// }
//  if ($gClient->getAccessToken()) {
//  $data = $google_service->people->get('me');
