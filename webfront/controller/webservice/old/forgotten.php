	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')  ) {
			if($api_info){
				...
				if (!isset($rest_var['email'])) {
					$json['message'] = $this->language->get('error_invalid_email');
				}elseif ((utf8_strlen($rest_var['email']) > 96) || !filter_var($rest_var['email'], FILTER_VALIDATE_EMAIL)) {
					$json['message'] = $this->language->get('error_invalid_email');
				...
				$customer_info = $this->model_account_customer->getCustomerByEmail($rest_var['email']);
				...
				if (!$json) {  
					...
					$this->model_account_customer->editCode($rest_var['email'], $code);
