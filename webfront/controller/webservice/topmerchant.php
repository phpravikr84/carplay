<?php
class ControllerWebserviceTopmerchant extends Controller {
		...
		if($api_info){ 
			...
			if (!empty($setting['merchant'])) {
				...
				foreach ($merchants as $merchant_id) {
					...
					$merchant_info = $this->model_catalog_merchant->getMerchantsById($merchant_id);
					...
					if ($merchant_info) {
						...
						$json['merchants'][] = array(
							...
							'href'        		=> $this->url->link('webservice/merchant', 'merchant_id=' . $merchant_info['merchant_id'])
						);
					}
