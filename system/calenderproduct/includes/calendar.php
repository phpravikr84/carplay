<?php
	...
	**************************************************************************/
	...
	class calendar
	{
		...
		*/
		public function __construct($db_server, $db_username, $db_password, $db_name, $table, $condition=false)
		{
			...
			if($this->condition == false)
			{
				...
				//$this->result = mysqli_query($this->connection, "SELECT * FROM calendar_product ");
			} else {
		...
		*/
		public function json_transform($js = true)
		{
			...
			while($this->row = mysqli_fetch_array($this->result, MYSQLI_ASSOC))
			{
				 ...
				 $event_seats = $this->row['seats'];
				 $event_discount = $this->row['discount'];
				 ...
				 if($js == true) 
				 {
					...
					// When allDay = false the allDay options appears on the script, when its true it doesnot appear 
					 if($event_url == 'false' && $event_allDay == 'false')
					 {
						 ...
						 // Stores Each Database Record To An Array (Without URL)
						$build_json = array('id' => $event_id, 'original_id' => $event_original_id, 'title' => $event_title, 'start' => $event_start, 'end' => $event_end, 'allDay' => $event_allDay, 'color' => $event_color, 'seats' => $event_seats, 'discount' => $event_discount);
						...
						array_push($this->json_array, $build_json);
						 ...
						 // Stores Each Database Record To An Array (Without URL)
						$build_json = array('id' => $event_id, 'original_id' => $event_original_id, 'title' => $event_title, 'start' => $event_start, 'end' => $event_end, 'color' => $event_color, 'seats' => $event_seats, 'discount' => $event_discount);
						...
						// Stores Each Database Record To An Array
						$build_json = array('id' => $event_id, 'original_id' => $event_original_id, 'title' => $event_title, 'start' => $event_start, 'end' => $event_end, 'allDay' => $event_allDay, 'color' => $event_color, 'url' => $event_url, 'seats' => $event_seats, 'discount' => $event_discount);
						...
						array_push($this->json_array, $build_json);
						 ...
						 if($event_allDay == 'false' && substr($event_url, -4, 1) == '.' || substr($event_url, -3, 1) == '.') { // domain top level checking
							...
							// Stores Each Database Record To An Array
							$build_json = array('id' => $event_id, 'original_id' => $event_original_id, 'title' => $event_title, 'start' => $event_start, 'end' => $event_end,  'color' => $event_color, 'url' => $event_url, 'seats' => $event_seats, 'discount' => $event_discount);
							...
							// Stores Each Database Record To An Array
							$build_json = array('id' => $event_id, 'original_id' => $event_original_id, 'title' => $event_title, 'start' => $event_start, 'end' => $event_end,  'allDay' => $event_allDay, 'color' => $event_color, 'url' => $event_url . $event_original_id, 'seats' => $event_seats, 'discount' => $event_discount);
							...
							// Stores Each Database Record To An Array
							$build_json = array('id' => $event_id, 'original_id' => $event_original_id, 'title' => $event_title, 'start' => $event_start, 'end' => $event_end, 'color' => $event_color, 'url' => $event_url . $event_original_id, 'seats' => $event_seats, 'discount' => $event_discount);
		...
		{	
			...
			$query = sprintf('UPDATE %s 
						...
						',
										...
										mysqli_real_escape_string($this->connection, $start),
										mysqli_real_escape_string($this->connection, $end),
										mysqli_real_escape_string($this->connection, htmlspecialchars($event['discount'])),
										mysqli_real_escape_string($this->connection, htmlspecialchars($event['seats'])),
		...
		* Returns true
		*/
		public function addEvent($title, $description, $start_date, $start_time, $end_date, $end_time, $color, $allDay, $url, $extra=false,$discount,$seats)
		{	
			// Avoid empty title
			if(strlen($title) == 0)
			{
			...
			{	
				...
				$query = sprintf('INSERT INTO %s 
							...
							',
											...
											mysqli_real_escape_string($this->connection, htmlspecialchars($discount)),
											mysqli_real_escape_string($this->connection, htmlspecialchars($seats)),
				...
				if(mysqli_affected_rows($this->connection) == 1)
				{
					if($repeat_method == 'no')
					{
						...
						$fields = array(
							...
							'discount' => mysqli_real_escape_string($this->connection, htmlspecialchars($discount)),
							'seats' => mysqli_real_escape_string($this->connection, htmlspecialchars($seats)),
...
$ical .= 'END:VCALENDAR'."\n";
		...
		*/
		...
		public function deleteCalenderData()
		{
			// Result Query
			$this->result = mysqli_query($this->connection, "TRUNCATE `calendar_product`");
			
		}
		
		public function updateCalenderData($id, $product_id){
		
			print $sql ="SELECT * FROM et_merchant_disc_product_wise WHERE merchant_id='".$id."' AND product_id='".$product_id."'";
			
			$this->result = mysqli_query($this->connection, $sql);	$i=1;
			while($this->row = mysqli_fetch_array($this->result, MYSQLI_ASSOC))
			{
				...
				$event_id = $this->row['repeat_id'];
				 ...
				 $event_seats = $this->row['seats'];
				 $event_discount = $this->row['discount'];
				 $event_merchantid = $this->row['merchant_id'];
				 $product_id = $this->row['product_id'];
				 $seats = $this->row['seats'];
				 $event_repeat_id = $this->row['repeat_id'];
				 
				 
				 $squery = sprintf('INSERT INTO calendar_product 
					SET 
					id = "%s",
					title = "%s",
					description = "%s",
					...
					$event_original_id, 
					mysqli_real_escape_string($this->connection, strip_tags($event_title)),
					mysqli_real_escape_string($this->connection, htmlspecialchars($event_description, ENT_COMPAT, 'UTF-8')),
					mysqli_real_escape_string($this->connection, htmlspecialchars($event_start)),
					mysqli_real_escape_string($this->connection, htmlspecialchars($event_end)),
					mysqli_real_escape_string($this->connection, $event_allDay),
					mysqli_real_escape_string($this->connection, htmlspecialchars($event_color)),
					mysqli_real_escape_string($this->connection, htmlspecialchars($event_url)),
					mysqli_real_escape_string($this->connection, htmlspecialchars($event_discount)),
					mysqli_real_escape_string($this->connection, htmlspecialchars($event_seats)),
					mysqli_real_escape_string($this->connection, htmlspecialchars($event_merchantid)),
					mysqli_real_escape_string($this->connection, htmlspecialchars($product_id)),
					mysqli_real_escape_string($this->connection, htmlspecialchars($event_repeat_id))
		);
		
		
		mysqli_query($this->connection, $squery);
		print '</br>'.$this->row['id'].$squery.'</br>';
		$i++;
		if($i==3){print '<pre>';print_r($this->row);exit;}
					  
			}
			
			
			//$this->result = mysqli_query($this->connection, "INSERT INTO `calendar_product` SELECT * FROM et_merchant_disc_product_wise WHERE merchant_id='".$id."' AND product_id='".$product_id."'");
