  ...
  <body>
    ...
    <div class="span8">
        <div class="table-bordered" style="border: 1px solid #ddd; padding: 10px;">
             ...
             <a href="index.php?merchant_id=<?php echo $info['merchant_id']?>" class="pull-right" style="margin-bottom: 20px;">Back to Discounts</a>
            
            <h3><?php echo ucfirst($info['title']); ?></h3>
            <p>
            	<span class="label">Discount Start:</span> <?php echo date('d/m/Y H:i', strtotime($info['start'])); ?> <br />
            	<span class="label">Discount End:</span> <?php echo date('d/m/Y H:i', strtotime($info['end'])); ?>
            </p>
            <p><?php echo $embed->oembed($formater->html_format($info['description'])); ?></p>
        	
            <input type="hidden" id="rep_id" value="<?php echo $info['repeat_id']; ?>" />

            <a href="#" class="pull-right" onclick="calendar.remove(<?php echo $info['id']; ?>)">Delete Discount</a>
    		<a href="edit_event.php?page=<?php echo $_GET['page']; ?>" class="pull-right mr-10">Edit Discount</a>
