	...
	class calendar
	{
		...
		*/
		public function __construct($db_server, $db_username, $db_password, $db_name, $table, $condition=false)
		{
			...
			if($this->condition == false)
			{
				...
				$this->result = mysqli_query($this->connection, "SELECT * FROM calendar ");
			} else {
		...
		*/
		...
		{	
			...
			$sql = "UPDATE `et_merchant_disc_product_wise` SET `discount` = '".$event['discount']."' WHERE merchant_id = '".$data['merchant_id']."' AND start='".$start."' AND end='".$end."'";
			
			$up_res = mysqli_query($this->connection, $sql);
...
$ical .= 'END:VCALENDAR'."\n";
		...
		*/
		...
		public function deleteCalenderData($id)
		{
			// Result Query
			$this->result = mysqli_query($this->connection, "TRUNCATE calendar");
			
			$this->result = mysqli_query($this->connection, "INSERT INTO `calendar` SELECT * FROM $this->table WHERE merchant_id=$id");
			
			
			
			 
			// return sprintf("DELETE FROM  calendar  WHERE merchant_id = %s", mysqli_real_escape_string($this->connection, $id));
